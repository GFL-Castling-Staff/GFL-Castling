	...
	protected void handleResultEvent(const XmlElement@ event) {
		...
		switch(int(gameSkillIndex[EventKeyGet]))
        {
			...
			case 7: {// XM8技能
				...
				if (character !is null) {
					...
					if (player !is null) {
						...
						tasker.add(XM8SkillTask(m_metagame, characterId, factionid, Pos_40mm));
					}
				}
				break;
			}
			...
			case 9: {// KCCO智能雷扫描阶段
				...
				if (character !is null) {
					...
					for(uint i=0;i<m_fnum;i++) {
						if(i==factionid) continue;
						array<const XmlElement@> affectedCharacter2;
						affectedCharacter2 = getCharactersNearPosition(m_metagame,pos_smartgrenade,i,7.0f);
			...
			case 17: {// 白教白憨憨普攻砸地
				...
				TaskSequencer@ tasker = m_metagame.getTaskManager().newTaskSequencer();
				tasker.add(DelayProjectileSet(m_metagame,0.6,characterId,character.getIntAttribute("faction_id"),"roarer_main.projectile",pos,true));
				break;
			}
